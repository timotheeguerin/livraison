export interface NormalizeOptionLength {
...
export function createFileHeader(options: NormalizeOptionLength): string {
  ...
  const maxLengthMap: Record<keyof NormalizeOptionLength, { maxLength: number; value: string }> = {
    fileName: {
      maxLength: 16,
      value: fileName,
    },
